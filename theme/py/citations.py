...
base_url = "https://scholar.google.es"
...
def fetchPage(url, delay=nap_time):
    sleep = delay + (delay * random.random())
    print "Sleeping for {} seconds".format(sleep)
    time.sleep(sleep)
    header = {'User-agent': agents[random.randint(0, len(agents)-1)]}
    #header = {'User-agent': agents[random.randint(0, len(agents)-1)], 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8', 'Referer': 'https://scholar.google.es/citations?view_op=view_citation&hl=en&user=M1FQwSUAAAAJ&citation_for_view=M1FQwSUAAAAJ:zYLM7Y9cAGgC'}
    ...
    html = r.text
    soup = bs(html, "lxml")
    if soup.text.find("not a robot") != -1:
        print "Encountered bot detection. Aborting."
        sys.exit()
    return soup
...
def parseArticles(page):
    ...
    for i, row in enumerate(rows):
        print "\tExtracting article {} of {}:".format(i+1, N)
        print row
...
def parseArticlePages(pages):
    ...
    for i, page in enumerate(pages):
        print "Parsing articles on page {} of {}".format(i+1, N)
...
def fetchAll(url):
    ...
    print "Found {} pages of articles".format(len(user_pages))

    print "Parsing user pages to extract articles."
    articles = parseArticlePages(user_pages)
    N = len(articles)
    print "Found {} articles in total.".format(N)

    print "Fetching citations."
    for i, article in enumerate(articles):
        print "Processing article {} of {}".format(i+1, N)
        citations = []
        if (not article['citations_link'] == ''):
            citation_pages = []
            print "\tFetching citation pages..."
            fetchPages(article['citations_link'], citation_pages)
            print "\tFound {} citation pages.".format(len(citation_pages))
            print "Parsing citations..."
            citations = parseCitationPages(citation_pages)
            print "Found {} citations.".format(len(citations))
            if len(citations) == 0:
                print article
                print citation_pages
